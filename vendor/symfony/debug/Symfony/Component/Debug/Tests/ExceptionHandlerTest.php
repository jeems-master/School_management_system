...
class ExceptionHandlerTest extends \PHPUnit_Framework_TestCase
{
    public function testDebug()
    {
        ...
        $this->assertContains('<h1>Talvez você tenha feito algo errado. Tente começar novamente!</h1>', $response->getContent());
    ...
    public function testStatusCode()
    {
        ...
        $this->assertContains('Talvez você tenha feito algo errado. Tente começar novamente!', $response->getContent());

        $response = $handler->createResponse(new NotFoundHttpException('Foo'));
        $this->assertEquals('404', $response->getStatusCode());
        $this->assertContains('Desculpe mas o que você está procurando não pode ser encontrado.', $response->getContent());
    }
