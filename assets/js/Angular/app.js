...
xhReq.open("GET", "api/csrf");
xhReq.send(null);

schoex.constant("CSRF_TOKEN", xhReq.responseText);

schoex.run(['$http', 'CSRF_TOKEN', function($http, CSRF_TOKEN) {
...
schoex.controller('registeration', function(dataFactory,$rootScope,$scope) {
  ...
  $scope.linkStudent = function(){
    $scope.modalTitle = "Vincular aluno com responsável";
  ...
  $scope.linkStudentFinish = function(student){
    ...
    do{
        var relationShip = prompt("Digite grau de parentesco ou relacionamento", "");
    }while(relationShip == "");
...
schoex.controller('teachersController', function(dataFactory,CSRF_TOKEN,$rootScope,$scope,$sce) {
  $scope.teachers = {};
  $scope.teachersTemp = {};
...
schoex.controller('studentsController', function(dataFactory,CSRF_TOKEN,$rootScope,$scope,$sce,$route) {
  $scope.students = {};
  $scope.studentsTemp = {};
  $scope.totalItemsTemp = {};
  $scope.classes = {};
...
schoex.controller('newsboardController', function(dataFactory,$routeParams,$sce,$rootScope,$scope,$filter) {
  $scope.newsboard = {};
  $scope.newsboardTemp = {};
  $scope.totalItemsTemp = {};
  $scope.views = {};
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('newsboard/'+id).then(function(data) {
      $scope.form = data;
      $scope.form.newsDate = $filter('date')($scope.form.newsDate * 1000, $rootScope.angDateFormat);
      $scope.changeView('edit');
      showHideLoad(true);
    });
  }
...
schoex.controller('examsListController', function(dataFactory,$rootScope,$scope,$sce,$filter) {
  $scope.examsList = {};
  $scope.classes = {};
  $scope.subjects = {};
  $scope.userRole ;
  ...
  $scope.notify = function(id){
    ...
    $scope.addMSCol = function(){
        var colTitle = prompt("Digite o título da coluna");
        if (colTitle != null) {
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('examsList/'+id).then(function(data) {
      $scope.changeView('edit');
      $scope.form = data;
      $scope.form.examDate = $filter('date')($scope.form.examDate * 1000, $rootScope.angDateFormat);
      showHideLoad(true);
    });
  }
...
schoex.controller('eventsController', function(dataFactory,$routeParams,$rootScope,$sce,$scope,$filter) {
  $scope.events = {};
  $scope.views = {};
  $scope.views.list = true;
  $scope.form = {};
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('events/'+id).then(function(data) {
      $scope.changeView('edit');
      $scope.form = data;
      $scope.form.eventDate = $filter('date')($scope.form.eventDate * 1000, $rootScope.angDateFormat);
      showHideLoad(true);
    });
  }
...
schoex.controller('assignmentsController', function(dataFactory,$rootScope,CSRF_TOKEN,$scope,$filter) {
  $scope.classes = {};
  $scope.subject = {};
  $scope.section = {};
  $scope.assignments = {};
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('assignments/'+id).then(function(data) {
      $scope.changeView('edit');
      $scope.form = data;
      $scope.form.AssignDeadLine = $filter('date')($scope.form.AssignDeadLine * 1000, $rootScope.angDateFormat);
      showHideLoad(true);
    });
  }
...
schoex.controller('paymentsController', function(dataFactory,$scope,$sce,$rootScope,$filter) {
  $scope.payments = {};
  $scope.students = {};
  $scope.classes = {};
  $scope.views = {};
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('payments/'+id).then(function(data) {
      $scope.form = data;
      $scope.form.paymentDate = $filter('date')($scope.form.paymentDate * 1000, $rootScope.angDateFormat);
      $scope.changeView('edit');
      showHideLoad(true);
    });
  }
...
schoex.controller('expensesController', function(dataFactory,$rootScope,$scope,$route,$filter) {
  $scope.expenses = {};
  $scope.views = {};
  $scope.views.list = true;
  $scope.form = {};
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('expenses/'+id).then(function(data) {
      $scope.changeView('edit');
      $scope.form = data;
      $scope.form.expenseDate = $filter('date')($scope.form.expenseDate * 1000, $rootScope.angDateFormat);
      showHideLoad(true);
    });
  }
...
schoex.controller('feeAllocationController', function(dataFactory,$rootScope,$scope,$route,$filter) {
  $scope.classes = {};
  $scope.feeTypes = {};
  ...
  $scope.edit = function(id){
    ...
    dataFactory.httpRequest('feeAllocation/'+id).then(function(data) {
      $scope.changeView('edit');
      $scope.form = data;
      $scope.form.allocationDate = $filter('date')(data.allocationDate * 1000, $rootScope.angDateFormat);
      showHideLoad(true);
    });
  }
...
schoex.controller('mobileNotifController', function(dataFactory,$rootScope,$scope) {
  ...
  dataFactory.httpRequest('mobileNotif/listAll').then(function(data) {
    $scope.messages = data;
    showHideLoad(true);
  });
