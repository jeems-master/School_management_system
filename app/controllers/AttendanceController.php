...
class AttendanceController extends \BaseController {
	...
	public function listAttendance(){
		...
		$vacationList = vacation::where('vacDate',$this->panelInit->dateToUnix(Input::get('attendanceDay')))->where('acYear',$this->panelInit->selectAcYear)->where('role','student')->get();

		if(Input::get('subjectId')){
			$attendanceArray = attendance::where('classId',Input::get('classId'))->where('subjectId',Input::get('subjectId'))->where('date',$this->panelInit->dateToUnix(Input::get('attendanceDay')))->get();
		}else{
			$attendanceArray = attendance::where('classId',Input::get('classId'))->where('date',$this->panelInit->dateToUnix(Input::get('attendanceDay')))->get();
		}
	...
	public function saveAttendance(){
		...
		if($this->data['panelInit']->settingsArray['attendanceModel'] == "subject"){
			$attendanceArray = attendance::where('classId',Input::get('classId'))->where('subjectId',Input::get('subject'))->where('date',$this->panelInit->dateToUnix(Input::get('attendanceDay')))->get();
		}else{
			$attendanceArray = attendance::where('classId',Input::get('classId'))->where('date',$this->panelInit->dateToUnix(Input::get('attendanceDay')))->get();
		}
		foreach ($attendanceArray as $stAttendance) {
		...
		while (list($key, $value) = each($stAttendance)) {
			...
			if(isset($value['attendance']) AND strlen($value['attendance']) > 0){
				if(!isset($attendanceList[$key])){
					...
					$attendanceN->date = $this->panelInit->dateToUnix(Input::get('attendanceDay'));
					...
					if($attendanceList[$key] != $value['attendance']){
						$attendanceN = attendance::where('studentId',$key)->where('date',$this->panelInit->dateToUnix(Input::get('attendanceDay')))->where('classId',Input::get('classId'))->first();
						$attendanceN->status = $value['attendance'];
						if($this->data['panelInit']->settingsArray['attendanceModel'] == "subject"){
							$attendanceN->subjectId = Input::get('subject');
						}
		...
		return $this->panelInit->apiOutput(true,"Frequência",$this->panelInit->language['attendanceSaved'] );
	}
	...
	public function getStats($date = ""){
		...
		if($this->data['users']->role == "admin" || $this->data['users']->role == "teacher"){
			$attendanceArray = attendance::where('date','>=',$startTime)->where('date','<=',$endTime)->orderBy('date')->get();
	...
	public function search(){
		...
		if(Input::has('classId') AND Input::get('classId') != "" ){
		...
		if(Input::has('attendanceDay') AND Input::get('attendanceDay') != ""){
			$sqlArray[] = "date='".$this->panelInit->dateToUnix(Input::get('attendanceDay'))."'";
		}
