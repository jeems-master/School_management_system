...
class ParentsController extends \BaseController {
	...
	function search($keyword,$page = 1){
		$toReturn = array();
		$toReturn['parents'] = User::where('role','parent')->where('activated','1')->where(function($query) use ($keyword){
		...
		$toReturn['totalItems'] = User::where('role','parent')->where('activated','1')->where(function($query) use ($keyword){
															$query->where('fullName','like','%'.$keyword.'%')->orWhere('username','like','%'.$keyword.'%')->orWhere('email','like','%'.$keyword.'%');
													})->count();
	...
	public function export(){
		...
		$xls = new Excel_XML('UTF-8', false, 'Responsaveis');
		$xls->addArray($data);
		$xls->generateXML('Responsaveis');
		exit;
	}
